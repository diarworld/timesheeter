[Назад к README](../README.md) · [Полная документация →](README.md)
...
- **Вы можете сменить тему:**
  - Используя переключатель <img align="center" src="light_switcher.gif" alt="light_switcher" width="25px"> в меню
...
- Задачи отображаются, если:
  ...
  - Были закреплены вручную (иконка 📌 рядом с ключом). <img align="center" src="pin.png" alt="pin" height="45px">
...
- **Общие встречи**  
  Встречи с числом участников больше 100 или с участием определённых рассылок (например, `ml.users@example.ru`, `ml.all.users@example.ru` и др.), а также встречи с названиями вроде "Coffee talk", "Доклады и Докладчики", "Domain Review", "ТДК", "TDK" — автоматически устанавливаются на задачу `PM-4`.
...
В разделе **Отчеты (Reports)** теперь доступны два режима просмотра:

1. **Время по дням (Time by Day)** — привычный вид, когда записи сгруппированы по датам (по умолчанию).

2. **Сводка по задачам (Issue Summary)** — новый вид, когда записи сгруппированы по задачам. Показывает:
   - Ключ задачи и её описание (до 2 строк)
   - Время, затраченное каждым членом команды на эту задачу
   - Общее время по задаче
   - Процент выполнения от плана (если настроены ожидаемые часы)

Переключение между режимами осуществляется кнопками над таблицей отчета.

> 💡 **Совет:** Режим "Сводка по задачам" удобен для понимания, на какие задачи тратится больше всего времени в команде.
...
- [Миграция конфигурации](CONFIGURATION_MIGRATION.md) — переход между версиями
