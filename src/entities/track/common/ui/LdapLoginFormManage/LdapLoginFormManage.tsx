...
export const LdapLoginFormManage: FC<TProps> = ({
  ...
  const handleSubmit = useCallback(async (values: { username: string; token: string; type?: string }) => {
    try {
  ...
  return (
    ...
    >
      <Form.Item name="username" label={message('ldap.auth.login')} rules={[{ required: true, type: 'email' }]}
      extra={
        <p>
          {message('ldap.auth.login.hint')}
        </p>
      }>
      ...
      <Form.Item
        ...
        extra={
          <p>
            {message('ldap.auth.password.hint')}
          </p>
        }
      >
        <Input type="password" />
